...
void SensorDataManager::AddSensorMeasurements(
  ...
  // sensors_是map(string, SensorPtr)结构，有每个传感器对应的一帧数据，保存相应传感器类型的最新的一帧
  // 如果sensors_里暂时没有保存过该传感器，插入该类型传感器
  if (it == sensors_.end()) {
    ...
    // sensors_里保存过这个传感器的数据,指针指向这个数据
    sensor_ptr = it->second;
  }

  //关于结构体：【sensor】中包含【sensor_frame】，相当于该传感器类型中有的一个数据
  // sensor_ptr里的队列【sensor_frame】插入最新的这一帧数据
...
void SensorDataManager::GetLatestFrames(
  ...
  frames->clear();
  // 查询每个传感器类型最新的一帧，插入到【vector】frames中
