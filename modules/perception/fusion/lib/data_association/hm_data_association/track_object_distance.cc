...
float TrackObjectDistance::Compute(const TrackPtr& fused_track,
  ...
  //Tracker中匹配过的lidar/radar/camera object
  ...
  //观测量sensor是lidar,计算观测量object和track中每个源object的最小距离
  if (IsLidar(sensor_object)) {
    if (lidar_object != nullptr) {
      // sensor和fusion_lidar,中心小于10米,预测一步,计算polygon中心点欧氏距离
    ...
    if (camera_object != nullptr) {
      //观测(sensor_object)和融合(lidar_object)ID是否一致
  ...
  //观测量sensor是radar, 和lidar/camera计算
...
float TrackObjectDistance::ComputeLidarLidar(
  ...
  double center_distance = (sensor_object->GetBaseObject()->center -
                            fused_object->GetBaseObject()->center)
                               .head(2)
                               .norm();
  if (center_distance > s_lidar2lidar_association_center_dist_threshold_) { //10
...
float TrackObjectDistance::ComputeLidarRadar(
  ...
  double center_distance = (sensor_object->GetBaseObject()->center -
                            fused_object->GetBaseObject()->center)
                               .head(2)
                               .norm();
  if (center_distance > s_lidar2radar_association_center_dist_threshold_) { //10米
...
float TrackObjectDistance::ComputeLidarCamera(
  ...
  if (cloud.size() > 0) {
    ...
    // similarity
    //没看懂,应该是投影lidar到camera,计算2D区域的相似性
...
float TrackObjectDistance::ComputePolygonDistance3d(
  ...
  //时间差,tracker往前预测一步,和object观测计算欧式距离
...
float TrackObjectDistance::ComputeEuclideanDistance(
  ...
  //(x*x,y*y).sum()  对应点相乘--cwiseProduct
