...
bool MlfEngine::Track(const MultiTargetTrackerOptions& options,
                      LidarFrame* frame) {
  // 0. modify objects timestamp if necessary=true
  // 时间戳赋值
  ...
  // 4. state filter in tracker if is main sensor=velodyne128
...
void MlfEngine::SplitAndTransformToTrackedObjects(
  ...
  for (size_t i = 0; i < objects.size(); ++i) {
    ...
    // 赋值给前景和背景障碍物
    // pointpillars貌似算出来的都是前景，计算结果直接is_background=false
    if (objects[i]->lidar_supplement.is_background) {
