...
bool ContiArsPreprocessor::Init() {
  ...
  ACHECK(lib::ConfigManager::Instance()->GetModelConfig(model_name,
                                                        &model_config));
  ACHECK(model_config->get_value("delay_time", &delay_time_)); //0.07
  return true;
}
// 雷达预处理
...
void ContiArsPreprocessor::SkipObjects(
  ...
  // 判断时间戳在[timestamp, timestamp+0.074]
  double timestamp = raw_obstacles.header().timestamp_sec() - 1e-6;
  for (const auto& contiobs : raw_contiobsobstacles.contiobs()) {
    double object_timestamp = .header().timestamp_sec();
    if (object_timestamp > timestamp &&
        object_timestamp < timestamp + CONTI_ARS_INTERVAL) { //0.074
