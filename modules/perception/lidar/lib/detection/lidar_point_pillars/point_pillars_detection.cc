...
bool PointPillarsDetection::Init(const DetectionInitOptions& options) {
  ...
  // FLAGS参数：false, 0.5, 0.5, 地址, 地址
  // Apollo/modules/perception/common/perception_gflags.cc
...
bool PointPillarsDetection::Detect(const DetectionOptions& options,
  ...
  // down sample the point cloud through filtering beams = false
  ...
  // down sample the point cloud through filtering voxel grid = false
  ...
  // shuffle points and cut off = false
