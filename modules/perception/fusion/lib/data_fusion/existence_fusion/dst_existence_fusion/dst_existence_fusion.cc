...
bool DstExistenceFusion::Init() {
  ...
  // Apollo/modules/perception/production/data/perception/fusion/dst_existence_fusion.pt
...
void DstExistenceFusion::UpdateWithMeasurement(
  ...
  double max_match_distance = options_.track_object_max_match_distance_; //4m
  
  // association_prob = 1-0/4 = 1
  ...
  // 距离衰减0-1  decay =  三维距离>60 ? 0.8 : 1
  double decay = ComputeDistDecay(obj, sensor_id, timestamp);
  // decay  = velocity_fact * confidence
