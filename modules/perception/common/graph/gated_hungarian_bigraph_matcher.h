...
void GatedHungarianMatcher<T>::MatchInit() {
  ...
  compare_fun_ = find_ret->second; // greater<T>()
  is_valid_cost_ = std::bind1st(compare_fun_, cost_thresh_); //小于4的关联值返回true，cost_thresh_ > 输入
...
void GatedHungarianMatcher<T>::ComputeConnectedComponents(
  ...
  // 建立二分图，阈值范围内的建立关系，放入vector内,前rows个为行，后面为列。
  ...
  // ！！连通子图components，相互连接的节点，即匈牙利匹配的增广路！！
  ConnectedComponentAnalysis(nb_graph, &components);

  // 拆分放入到两个vector里，大小是子图的size
...
void GatedHungarianMatcher<T>::UpdateGatingLocalCostsMat(
  ...
  // 二分子图转换成矩阵local_costs，空的位置设为bound_value_
