...
bool ObjPostProcessor::AdjustCenterWithGround(const float *bbox,
                                              ...
                                              const float *plane,
                                              float *center) const {
  // 计算识别到的bbox 和根据center计算得到的顶点映射到2d得到的bbox的重叠比例 
  // params_.iou_good=0.5 sampling_ratio_low=0.1 weight_iou=3 max_nr_iter=5 learning_r=0.2
  ...
  while (!stop) {
    // 将像素坐标转换根据为图像坐标系 2d-2d
    common::IProjectThroughIntrinsic(k_mat_, center, x);
    x[0] *= common::IRec(x[2]);
    x[1] *= common::IRec(x[2]);
    // 将图像坐标系结合地面信息转化为camera坐标系 2d-3d  center_test为计算得到的3d坐标
...
bool ObjPostProcessor::PostRefineCenterWithGroundBoundary(
  ...
  // 是否底部截断了 sampling_ratio_low=0.1  车辆的bbox高度接近于图片高度
...
int ObjPostProcessor::GetDepthXPair(const float *bbox, const float *hwl,
  ...
  for (int i = 0; i < 4; ++i) {
    // 对pt坐标系进行转换 先转换到camer坐标系 再转换到像素坐标系
