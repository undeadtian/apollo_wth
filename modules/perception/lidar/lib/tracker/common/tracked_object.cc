...
void TrackedObject::ComputeShapeFeatures() {
  ...
  shape_features_full.resize(histogram_bin_size * 7)//10*7 vector
  // 前景是box的八个顶点初始化FeatureDescriptor,背景是整块点云
  FeatureDescriptor fd(&(object_ptr->lidar_supplement.cloud));
  // 直方图：我理解为每个点坐标xyz在整个聚类点云里面的分布特征
  fd.ComputeHistogram(static_cast<int>(histogram_bin_size),
                      shape_features_full.data());

  size_t feature_len = histogram_bin_size * 3;//10*3
  // shape_features放xyz三个方向上各10个单位的数量值, 共30个
  shape_features.clear();
